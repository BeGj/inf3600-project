  ...
  return (
    <div className="generated-list">
      ...
      {overlays.map((overlay) => (
        <div
          key={overlay.maskId}
          className={`generated-list-item${overlay.visible ? "" : " hidden"}`}
        >
          <img
          ...
          <span className="label" title={overlay.prompt}>
            {overlay.prompt.length > 28
              ? overlay.prompt.slice(0, 26) + "…"
              : overlay.prompt}
          </span>
          <div className="actions">
            ...
            <button onClick={() => onZoom(overlay.bbox)} title="Zoom to extent">
              ⊕
            </button>
            <button
              onClick={() => onEdit(overlay)}
              title="Edit — restore mask and parameters"
            >
              ✏
            </button>
            <button onClick={() => onRemove(overlay.maskId)} title="Remove">
              ×
            </button>
          </div>
        </div>
